...
class NominalAttribute(object):
  ...
  def check_value(self, value):
    ...
    elif self.values == 'numeric':
	  # confirming values of type numeric, are actually numeric
	  try:
	    x = float(value)
	    if x > self.num_max:
	      self.num_max = x
  ...
  def binarize(self, value):
	...
	# if data is numeric, instead of binarizing, we normalize to between 0 and 1
	# based on the max and min for the attribute
    if self.values == 'numeric':
	  try:
	    x = float(value)
	  except ValueError:
	    raise BadLine('Expected value %s to be numeric %s' % (value, self.name)) 
      # normal_val = (value - min)/(max - min)
	  normal_val = (x - self.num_min)/(self.num_max - self.num_min)
	  return [normal_val]
    else:
      assert value in self.values + ['?',], ('Unsupported value %s for feature %s' % (value, self.name))
...
class Perceptron(object):
  ...
  def __init__(self, data):
    ...
    self.models = [
        perceptron.learn([(features, outputs[i]) for features, outputs in data], 100, False)
      ...
      try:
        if line[0] == '%':
          ...
          if match:
		    ...
		    num_match = re.search(r'@ATTRIBUTE\s+(\S+)\s+(numeric)', line, re.I)
		    if num_match:
			  # adding values as string 'numeric' initially
			  name, values = num_match.groups()
			  attributes.append(NominalAttribute(name, values))
		    else:
